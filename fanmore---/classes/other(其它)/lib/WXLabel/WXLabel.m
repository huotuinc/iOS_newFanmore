...
- (void)drawRect:(CGRect)rect
{
    ...
    //自定义当前超链接文本颜色
    if ([self.delegate respondsToSelector:@selector(linkColorWithWXLabel:)]) {
        self.linkColor = [self.delegate linkColorWithWXLabel:self];
    }
    
    //当前文本超链接文字手指经过的颜色默认为greenColor
    self.passColor = [UIColor greenColor];
    //自定义当前超链接文本颜色
    if ([self.delegate respondsToSelector:@selector(passColorWithWXLabel:)]) {
        self.passColor = [self.delegate passColorWithWXLabel:self];
    }
    if (self.text == nil) {
        return;
    }
...
//    CTFontRef font = CTFontCreateWithName(CFSTR("Georgia"), 15, NULL);
    ...
    //----------------------设置链接文本的颜色-------------------
    //判断当前链接文本表达式是否实现
    if ([self.delegate respondsToSelector:@selector(contentsOfRegexStringWithWXLabel:)] && [self.delegate contentsOfRegexStringWithWXLabel:self] != nil)
    {
        //获取所有的链接文本
...
#warning 设置图片的大小
                    imageDrawRect.size = CGSizeMake(self.font.pointSize * 1.0, self.font.pointSize * 1.0);
...
- (NSArray *)imagesOfRegexStrArray
{
    ...
    if ([self.delegate respondsToSelector:@selector(imagesOfRegexStringWithWXLabel:)]) {
        regex = [self.delegate imagesOfRegexStringWithWXLabel:self];
    }
...
- (void)replaceImageText
{
    ...
    for (NSString *imageUrl in [self imagesOfRegexStrArray]) {
        ...
        //设置空格的属性
        [imageAttributedString addAttribute:WXLABEL_IMAGE_NAME value:imgName range:NSMakeRange(0, 1)];
...
- (NSArray *)contentsOfRegexStrArray
{
    ...
    NSString *regex = [self.delegate contentsOfRegexStringWithWXLabel:self];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    {
        //判断当前代理方法是否实现
        if ([self.delegate respondsToSelector:@selector(toucheEndWXLabel:withContext:)]) {
            //获取当前点击字符串
            NSString *context = [[self.attrString string] substringWithRange:range];
            //调用点击开始代理方法
            [self.delegate toucheEndWXLabel:self withContext:context];
        }
    }
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    {
        //判断当前代理方法是否实现
        if ([self.delegate respondsToSelector:@selector(toucheBenginWXLabel:withContext:)]) {
            //获取当前点击字符串
            NSString *context = [[self.attrString string] substringWithRange:range];
            //调用点击开始代理方法
            [self.delegate toucheBenginWXLabel:self withContext:context];
        }
    }
